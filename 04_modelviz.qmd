...
```{r}
predicts_viz %>%
    filter(StanceH %in% c("Pos", "Neg"), is.na(StanceS), is.na(ImpositionH)) %>%
...
```{r}
comparisons_viz %>%
    filter(term == "StanceS", contrast %in% c("T -"), is.na(StanceH), is.na(StanceS)) %>%
    ggplot(aes(x = group, y = estimate, col = contrast)) +
    geom_hline(yintercept = 0) +
    facet_grid(Power ~ RelDist,
               labeller = as_labeller(c(levels2text_RelDist, levels2text_pow))) +
...
```{r}
shaplist = list()
examples_to_use = c(71, 104, 12, 84, 340, 331, 113, 124, 93, 234, 344, 122, 97, 224, 131, 112, 124, 133, 237, 344, 122, 97, 224, 88, 61, 43, 127, 335, 73, 261, 72, 142, 90, 215, 163)
